...
- [ ] Clone `eshop-sut`, run backend locally, confirm all 3 target endpoints respond (`GET /api/products/:id`, `POST /api/forgot-password`, `POST /api/apply-coupon`).
- [ ] Install/verify JMeter (non-GUI mode) — `jmeter -n -t <plan>.jmx -l <out>.jtl -e -o <report-dir>`.
- [ ] Install/verify resource monitor (htop or equivalent).
- [ ] 🔴 MANUAL — Capture hardware report screenshot (dxdiag/screenfetch) + spec table, one-time for the whole assignment (Section 6 Task 1 "Run as completely as possible" bullet; Section 11 anti-cheat requires hostname to match previous HW deployments; Section 14 lists it in the zip).
